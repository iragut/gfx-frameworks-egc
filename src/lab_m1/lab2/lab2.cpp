...
void Lab2::Init()
{
    ...
    }
    {
		...
		mesh->LoadMesh(PATH_JOIN(window->props.selfDir, RESOURCE_PATH::MODELS, "blender"), "untitled.obj");
		meshes[mesh->GetMeshID()] = mesh;
    }
...

    {
        vector<VertexFormat> vertices
        {
            VertexFormat(glm::vec3(-1, -1,  1), glm::vec3(1, 0, 0), glm::vec3(0.2, 0.8, 0.6)), //0
            VertexFormat(glm::vec3(1, -1,  1), glm::vec3(0, 1, 0), glm::vec3(0.2, 0.8, 0.6)), // 1
            VertexFormat(glm::vec3(-1,  1,  1), glm::vec3(0, 0, 1), glm::vec3(0.2, 0.8, 0.6)), // 2
			VertexFormat(glm::vec3(0,  0, -1), glm::vec3(0, 0, 1), glm::vec3(0.2, 0.8, 0.6)), // 3
        };
...

    {
        vector<VertexFormat> vertices
        {
            ...
            VertexFormat(glm::vec3(1, -1,  0), glm::vec3(0, 1, 0), glm::vec3(0.2, 0.8, 0.6)), // 1
            VertexFormat(glm::vec3(-1,  1,  0), glm::vec3(0, 0, 1), glm::vec3(0.2, 0.8, 0.6)), // 2
            VertexFormat(glm::vec3(1,  1,  0), glm::vec3(0, 0, 1), glm::vec3(0.2, 0.8, 0.6)), // 3
        };
...
void Lab2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(0, 0.5, 0), glm::vec3(0.25f));

    // TODO(student): Draw the square
	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(0.7, 0.5, 0), glm::vec3(0.25f));

    // Create a transformation matrix
    glm::mat4 modelMatrix = glm::mat4(1);  // Initialize to identity matrix
    ...
    // Render the mesh with the transformation matrix
...
void Lab2::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_F2) {
		cullFace = cullFace ^ GL_FRONT ^ GL_BACK;
	}
